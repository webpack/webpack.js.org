...
|**`banner`**|`{Boolean\|String\|Function}`|`/*! For license information please see ${filename}.js.LICENSE */`|
banner 文本会在原文件的头部指出被提取的文件。
...
banner 文本会在原文件的头部指出被提取的文件。会在源文件加入该信息。`string`，或者`function (string) -> string`会在提取已经被存储注释的时候被调用。注释会被覆盖。
