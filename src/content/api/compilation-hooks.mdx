...
<span id="list-of-asset-processing-stages"></span>
...
هذا الـhook من نوع `SyncBailHook`؛ لذلك تُعتمد نتيجة أول tap يعيد قيمة غير `undefined`، ولا تُستدعى بقية taps في ذلك الاستدعاء.

## HtmlModulesPlugin.getCompilationHooks(compilation)

<Badge text="5.109.0+" />

عند تفعيل [`experiments.html`](/configuration/experiments/#experimentshtml)، يتيح `HtmlModulesPlugin` الداخلي مجموعة hooks خاصة بكل عملية تصريف لحقن صفحات HTML الناتجة وتحويلها. يمكنك الوصول إليها عبر الدالة الساكنة `getCompilationHooks(compilation)`:
...
class MyHtmlPlugin {
  apply(compiler) {
    compiler.hooks.thisCompilation.tap("MyHtmlPlugin", (compilation) => {
      ...
      // سجّل taps على الـhooks هنا
    });
  }
}
```

W> لا تُسجّل هذه الـhooks إلا عند تفعيل `experiments.html`، وقد تتغير ما دامت ميزة HTML تجريبية.

### injectTags

`AsyncSeriesWaterfallHook<[HtmlTagDescriptor[], { outputName, html }]>`

تُستدعى مع قائمة الوسوم الإضافية المراد حقنها في كل صفحة، وتكون فارغة في البداية، ومع HTML الحالي. أضف واصفات الوسوم إلى القائمة ثم أعدها، وسيتولى webpack تحويلها إلى HTML ووضعها في أماكنها. تعمل قبل [`output.html.csp`](/configuration/output/#outputhtmlcsp)، ولذلك تُحسب تجزئات وسوم `<script>` و`<style>` المضمنة المحقونة كما تُحسب لوسوم الصفحة الأصلية.

يقبل كل `HtmlTagDescriptor` الخصائص التالية:

- `tag` ‏(`string`): اسم الوسم، مثل `'script'` أو`'link'` أو`'meta'`.
- `attrs` ‏(`object`): السمات؛ تُخرج `true` سمة منطقية بلا قيمة، بينما تحذفها `false` و`undefined`.
- `children` ‏(`string`): المحتوى الداخلي، ويُتجاهل مع العناصر الفارغة مثل `<link>` و`<meta>`.
- `injectTo` ‏(`'head' | 'body' | 'head-prepend' | 'body-prepend'`): موضع الحقن، وقيمته الافتراضية `'head'`.
- `voidTag` ‏(`boolean`): يجبر الوسم على أن يكون عنصرًا فارغًا بلا وسم إغلاق. وإذا حُذف، تُستنتج قيمته من اسم الوسم.
...
تُستدعى مع وسوم `<script>` و`<link>` و`<style>` و`<meta>` الموجودة في الصفحة، سواء أنشأها webpack أم حُقنت لاحقًا، على هيئة واصفات قابلة للتعديل. عدّل `attrs` لإضافة `nonce` أو سمة `data-*` أو لتبديل `defer` و`async`، أو اضبط `remove: true`، أو غيّر `injectTo` لنقل الوسم بين `<head>` و`<body>`؛ وسيعيد webpack كتابة الوسوم المعدلة. استخدم [`injectTags`](#injecttags) لإضافة وسوم جديدة، ولا تعِد ترتيب المصفوفة نفسها.

### transformHtml

`AsyncSeriesWaterfallHook<[string, { outputName: string }]>`

تُستدعى مع HTML النهائي لكل صفحة ناتجة بعد حل جميع العناصر النائبة وقبل كتابة الصفحة مباشرةً. أعد HTML بعد تحويله، كتصغيره مثلًا:
...
تُستدعى مرة واحدة بعد اكتمال أصل HTML لكل صفحة، لتعمل كإشعار بعد الإخراج.
