...
Expanding a requirement into the ones it implies works as it does above; the other use is attaching the runtime module that implements it, where the returned value stops any further tap for that requirement:
...
W> The hook is synchronous, so a tap cannot await anything. To attach a runtime module whose implementation is loaded on demand, hand it to `compilation.addLazyRuntimeModule(chunk, load, create)` instead and read whatever you need from the requirement set before queueing it.
