...
- `stage: Stage` — a stage to tap into (see the [list of supported stage](#list-of-asset-processing-stages) below)
- `additionalAssets?: boolean | (assets, [callback]) => (void | Promise<void>)` — a callback for additional assets ([see below](#additional-assets))

**Callback parameters:**
- `assets: { [pathname: string]: Source }` — a map object of assets, where key is the asset's pathname and the value is the source content of the asset
