...
`this.loadModule` in loader context uses commonjs resolve rules by default. Use `this.getResolve` with an appropriate `dependencyType`, e. g. `esm` or `commonjs` or a custom one before using a different semantic.
