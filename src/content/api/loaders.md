...
- `context` must be an absolute path to a directory. This directory is used as starting location for the resolving.
- `request` is the request that should be resolved. Usually either relative request like `./relative`, module requests like `module/path`, or root requests `/some/path` are used, but absolute paths are also possible as requests.
...
As `options` all webpack `resolve` options are possible. The options are merged with the configured `resolve` options. Note that `"..."` can be used in arrays to extend the value from `resolve` options, e. g. `{ extensions: [".sass", "..."] }`.

`options.dependencyType` is an additional option. It allows to specify the type of dependency, which is used to resolve `byDependency` from the `resolve` options.
