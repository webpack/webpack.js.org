...
with [`experiments.executeModule`](/configuration/experiments/#experimentsexecutemodule).
...
module.exports = {
  module: {
    rules: [
      {
        test: /target-file\.js$/i,
        use: ['./a-pitching-loader.js', './a-normal-loader.js'],
      },
    ],
  },
};
...
```js
module.exports = function (source) {
  // a dumb normal loader returning the source
  return source;
};
```

There are multiple things to watch in the above example:
