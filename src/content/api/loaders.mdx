...
Add a existing file as dependency of the loader result in order to make them watchable. For example, [`sass-loader`](https://github.com/webpack-contrib/sass-loader), [`less-loader`](https://github.com/webpack-contrib/less-loader) uses this to recompile whenever any imported `css` file changes.
...
Add a non-existing file as dependency of the loader result in order to make them watchable. Very similar to `addDependency`, but handles creation of files during compilation before watchers are attached correctly.
