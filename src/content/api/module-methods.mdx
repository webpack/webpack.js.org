...
// Webpack handles module caching internally.
```
...
W> These syntaxes are legacy. We highly recommend using ES6 Modules for modern applications.
...
W> The `name` argument does not affect the module id. webpack registers it as a local module, so other `define` or `require` calls in the same file can reference it.
...
W> `require.include()` emits a `RequireIncludeDeprecationWarning` ("require.include() is deprecated and will be removed soon.") by default. Set [`module.parser.javascript.requireInclude`](/configuration/module/#moduleparserjavascript) to `true` to silence it, or to `false` to disable the feature.
