...
Then import the webpack module in your Node.js script:
...
W> Multiple configurations are **run in parallel** by default
(`parallelism: Infinity`). Use the [`parallelism`](/configuration/configuration-types/#parallelism) option on the
configuration array or [`dependencies`](/configuration/configuration-types/#dependencies) in a configuration to limit or order them.
...
| `shutdown`          | `AsyncSeriesHook<[]>`                   | <Badge text="5.110.0+" /> Called when the compilers are closing. A `MultiHook` over the children's `shutdown` hooks; taps receive no arguments. |
...
[`fs`](https://nodejs.org/api/fs.html) interface: it must provide `mkdir`,
`writeFile`, `stat` and `readFile`; `join`, `relative` and `dirname` are optional.
