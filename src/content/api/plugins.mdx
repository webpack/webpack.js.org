...
to provide a similar plugin interface. Many objects in webpack expose a
`hooks` object of tapable hooks. Each hook exposes `tap` and, depending on its type, `tapAsync` and `tapPromise` methods
...
provide them, is crucial. The objects that expose hooks (e.g. the compiler),
...
To report progress, a plugin gets the reporter with `ProgressPlugin.getReporter(compiler)` and calls it from a normal `tap`. `getReporter` returns `undefined` when no `ProgressPlugin` is active:
