...
<img src={banner} alt="webpack 5.110 release" />
...
- A library build no longer gets a source map by default in `development`. The default [`devtool`](/configuration/devtool/) wraps each module in `eval`, and a library is read by another bundler, which the wrapper then hides `import.meta` from. This applies when the build emits a `module` or `modern-module` library, and with [`experiments.futureDefaults`](/configuration/experiments/#experimentsfuturedefaults) to a library of any type. Setting `devtool` yourself still does what it says.
