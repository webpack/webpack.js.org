...
ربما لاحظت أن ملفات تكوين webpack نادرًا ما تبدو متطابقة تمامًا. والسبب أن **ملف تكوين webpack هو ملف JavaScript يصدّر [تكوينًا](/configuration/) خاصًا بـ webpack.** بعد ذلك يعالج webpack هذا التكوين بناءً على الخصائص المعرّفة لديه.

ولأنه ملف Node.js قياسي، يمكنك القيام بالآتي:
- استيراد ملفات أخرى باستخدام `require(...)` أو `import`.
- استخدام أدوات من npm باستخدام `require(...)` أو `import`.
- كتابة تعبيرات التحكم الخاصة بـ JavaScript مثل `?:` و `if` و `for` وغيرها.
- تعريف الثوابت أو المتغيرات للقيم التي تتكرر كثيرًا.
- كتابة دوال وتنفيذها لتوليد أجزاء من التكوين.

استخدم هذه الميزات وقتما تراه مناسبًا.
