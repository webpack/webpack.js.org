...
__Why?__ With this you can import required libraries or files that aren't modified (i.e. Bootstrap, jQuery, images, etc) inside `vendor.js` and they will be bundled together into their own HTML. Content hash remains the same, which allows the browser to cache them separately thereby reducing load time.
