...
In the example above, the `ProgressPlugin` customizes how build progress is reported, and [`output.html`](/configuration/output/#outputhtml) generates an HTML file for your application with all your generated bundles injected into it — webpack does that itself, so no plugin is needed for it.
