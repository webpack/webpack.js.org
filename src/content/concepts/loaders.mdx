...
<<<<<<< HEAD
- [配置方式](#configuration)（推荐）：在 __webpack.config.js__ 文件中指定 loader。
- [内联方式](#inline)：在每个 `import` 语句中显式指定 loader。
- [CLI 方式](#cli)：在 shell 命令中指定它们。
...
这会对 `.jade` 文件使用 `pug-loader`，以及对 `.css` 文件使用 [`style-loader`](/loaders/style-loader) 和 [`css-loader`](/loaders/css-loader)。
