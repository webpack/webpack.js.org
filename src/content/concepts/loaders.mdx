...
- Loaders can be chained. Each loader in the chain applies transformations to the processed resource. A chain is executed in reverse order. The first loader passes its result (resource with applied transformations) to the next one, and so forth. Finally, webpack expects the last loader in the chain to return content that matches the module's `type`: JavaScript for `javascript/*` modules, CSS for `css/*`, JSON for `json` and raw content for `asset/*`.
