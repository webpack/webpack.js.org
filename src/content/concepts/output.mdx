...
If your configuration creates more than a single "chunk" (as with multiple entry points or when using [`optimization.splitChunks`](/configuration/optimization/#optimizationsplitchunks), the `SplitChunksPlugin`), you should use [substitutions](/configuration/output/#outputfilename) to ensure that each file has a unique name.
