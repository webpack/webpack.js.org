...
The `ProgressPlugin` is used to customize how progress should be reported during compilation, and [`output.html`](/configuration/output/#outputhtml) generates an HTML file including the `my-first-webpack.bundle.js` file using a `script` tag — see [Native HTML](/guides/native-html/).
