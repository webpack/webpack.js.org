...
T> If the default webpack `minimizer` has been overridden (such as to customise the `minimizer-webpack-plugin` options), make sure to configure its replacement with `sourceMap: true` to enable SourceMap support.
