...
export default {
  ...
  entry: {
    ...
    personal: {
      ...
      worker: true, // تمييزها كنقطة دخول لـ worker؛ متاح منذ webpack 5.108.0.
    },
  },
};
```
...
T> يميز الخيار `worker`، المضاف في webpack 5.108.0، نقطة الدخول بوصفها worker، فيستخدم ملف الإخراج الخاص بها [`output.workerChunkFilename`](/configuration/output/#outputworkerchunkfilename) بدلًا من اسم ملف chunk المعتاد. يضبط webpack هذا الخيار تلقائيًا لنقاط الدخول التي ينشئها عبر `new Worker(new URL(...))`، لذلك نادرًا ما تحتاج إلى ضبطه يدويًا.
