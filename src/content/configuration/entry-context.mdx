...
export default {
  ...
  entry: {
    ...
    personal: {
      ...
      worker: true, // اعتبارها نقطة دخول لـ worker، متاح منذ webpack 5.108.0.
    },
  },
};
```
...
T> تجعل الراية `worker`، المتاحة منذ webpack 5.108.0، نقطة الدخول خاصةً بـ worker؛ وبذلك يستخدم ملف الإخراج [`output.workerChunkFilename`](/configuration/output/#outputworkerchunkfilename) بدل اسم ملف chunk المعتاد. يضبطها webpack تلقائيًا لنقاط الدخول التي ينشئها من `new Worker(new URL(...))`، لذلك نادرًا ما تحتاج إلى ضبطها يدويًا.
