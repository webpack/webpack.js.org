...
بدءًا من webpack 5.109.0، تكون قيمة الخيار الافتراضية `'auto'`: يُفعّل دعم HTML المدمج ما لم توجد قاعدة في `module.rules` تحتوي على loader أو نوع وحدة صريح يطابق ملفات `.html`. وبهذا تستمر إعدادات `html-loader` الحالية في العمل من دون تعديل. وعند تفعيل [`experiments.futureDefaults`](#experimentsfuturedefaults)، تُحسم القيمة إلى `true`.
...
W> **هذه الميزة تجريبية وجزئية.** نفّذ webpack 5.107 الدور الذي يؤديه `html-loader`: عند استيراد ملف HTML من JavaScript، تمر مراجع وسومه عبر pipeline الخاصة بـwebpack. وبدءًا من 5.108، يمكن استخدام ملف `.html` كنقطة دخول، كما تُحل نقطة الدخول الافتراضية `./src` إلى `index.html` عند تفعيل هذه التجربة؛ راجع قسم [HTML كنقطة الدخول الافتراضية](#html-as-the-default-entry) أدناه. ما يزال العمل جاريًا للوصول إلى تكافؤ كامل مع `html-webpack-plugin`، ويمكن متابعة الجهد في issue [#536](https://github.com/webpack/webpack/issues/536).

T> يمكنك تخصيص محلل HTML عبر [`module.parser.html`](/configuration/module/#moduleparserhtml): استخدم [`sources`](/configuration/module/#moduleparserhtmlsources) لتعطيل استخراج سمات URL أو تخصيصه، و[`template`](/configuration/module/#moduleparserhtmltemplate) لتحويل مصدر HTML قبل تحليله.
