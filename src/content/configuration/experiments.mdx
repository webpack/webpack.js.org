...
Since webpack 5.109.0, the option defaults to `'auto'`: the built-in TypeScript support is enabled when the running Node.js provides `module.stripTypeScriptTypes` (>= 22.6) and no `module.rules` entry with a loader (e.g. `ts-loader`, `swc-loader`) matches `.ts`/`.mts`/`.cts` files. [`experiments.futureDefaults`](#experimentsfuturedefaults) resolves it to `true`.
