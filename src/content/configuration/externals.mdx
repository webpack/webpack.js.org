...
```javascript
import * as __WEBPACK_EXTERNAL_MODULE_$_ from '$';

const jq = __WEBPACK_EXTERNAL_MODULE_$_['default'];
jq('.my-element').animate(/* ... */);
```

Note that there will be a `import` statement in the output bundle.
...
const jq = createRequire(import.meta.url)('$');
jq('.my-element').animate(/* ... */);
```
