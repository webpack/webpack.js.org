  ...
  module: {
    generator: {
      ...
      "css/auto": {
        ...
        // Since webpack 5.108.0, [hash] here resolves to the local ident hash; use [modulehash] for the module hash. An inline digest/length form is also supported, e.g. [hash:base64:8].
...
When `exportsConvention` is a function, it may return either a `string` or a `string[]`. Returning an array exports the local class under every name in the array. This is useful when you want to expose the same class under several aliases without writing two rules.
...
T> To filter specific imports, you can use Webpack's built-in [IgnorePlugin](/plugins/ignore-plugin/). A per-import `filter` option is not supported.
...
- `style` - store CSS in JS file and inject it into the DOM at runtime through a `<style>` element (available since 5.106.0).
...
W> Deprecated since webpack 5.109.0: use the [`importMeta.webpackContext`](#moduleparserjavascriptimportmeta) object option instead. When both are set, `importMeta.webpackContext` wins.
...
Hand out a spec-compliant [Module Namespace Exotic Object](https://tc39.es/ecma262/#sec-module-namespace-exotic-objects) for `import * as ns` and `import()` of this module, instead of the plain exports object webpack hands out by default.

- Type: `boolean`
- Default: `false`
- Available: 5.111.0+

The plain exports object already reports `[object Module]`, carries the same live bindings, and throws a `TypeError` on writing to or deleting an export, so most code cannot tell the two apart. What the spec object adds is the remaining invariants: `Object.getPrototypeOf(ns)` is `null` rather than `Object.prototype`, and `Object.isExtensible(ns)` is `false`. Reach for it when something in the graph checks those — a conformance test, or a library that feature-detects a real namespace — not as a general setting.

W> Set it on the module being **imported**, not on the importer — webpack asks each module whether the namespace it hands out is a spec object. Enabling it for a file that only does `import * as ns` changes nothing.

Because it is implemented with a `Proxy` and costs extra runtime code, enable it per module — usually through [`Rule.parser`](#ruleparser) — rather than globally:
...
W> The target environment must support `Proxy`, which rules out ES5 targets.
...
Parse TypeScript syntax. Defaults to [`experiments.typescript`](/configuration/experiments/#experimentstypescript), so enabling the experiment turns this on for you; set it here to override that per rule.

- Type: `boolean`

#### module.parser.javascript.deferImport

Parse the TC39 [deferred module evaluation](https://github.com/tc39/proposal-defer-import-eval) proposal (`import defer * as ns from "..."`). Defaults to [`experiments.deferImport`](/configuration/experiments/#experimentsdeferimport).

- Type: `boolean`

#### module.parser.javascript.sourceImport

Parse the TC39 [source phase imports](https://github.com/tc39/proposal-source-phase-imports) proposal (`import source x from "..."`). Defaults to [`experiments.sourceImport`](/configuration/experiments/#experimentssourceimport).

- Type: `boolean`

#### Syntax toggles

Each of these turns parsing of one module syntax on or off. They take a `boolean`, and **they do not all default the same way** — three of them are off, or warn, until you say otherwise:

| Option           | Syntax                                                    | Default                                                                               |
| ---------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `amd`            | AMD — `define()`, `require([...], cb)`                    | on                                                                                    |
| `browserify`     | browserify-specific handling                              | on                                                                                    |
| `commonjs`       | CommonJS — `require()`, `module.exports`                  | on                                                                                    |
| `harmony`        | ECMAScript modules — `import` / `export`                  | on                                                                                    |
| `import`         | `import()`                                                | on                                                                                    |
| `requireContext` | `require.context()`                                       | on                                                                                    |
| `requireEnsure`  | `require.ensure()`                                        | on                                                                                    |
| `requireInclude` | `require.include()`                                       | on, and warns that it is deprecated unless you set it explicitly                      |
| `requireJs`      | RequireJS specifics — `require.config`, `require.version` | **off** unless set to `true`                                                          |
| `system`         | System.js — `System.import`, `System.register`            | **off** unless set to `true`                                                          |
| `node`           | Node.js built-ins — `__dirname`, `__filename`, `global`   | takes the same shape as [`node`](/configuration/node/); `false` disables the handling |
...
Since webpack 5.110.0 the resource can also be matched with [`glob`](#ruleglob), which is OS-independent, and with [`descriptionRelativePath`](#ruledescriptionrelativepath), which is the resource's path inside its own package.
...
A [`Condition`](#condition) matched against the path of the module relative to the directory of its description file (usually the closest `package.json`), for example `./lib/button.js`. The path always uses forward slashes, so the same rule matches on every operating system.
...
The condition does not apply when a [match resource](/api/loaders/#inline-matchresource) replaced the resource, since the description file of the original request no longer describes it.
...
`glob` combines with the other conditions: [`test`](#ruletest), [`include`](#ruleinclude) and [`exclude`](#ruleexclude) still have to match too. It is also available inside a [`Condition`](#condition) object, which lets you use it wherever a condition is accepted:
...
W> This option was **removed** in webpack 5 and is rejected by the configuration validation.

Use [`Rule.use`](#ruleuse) instead.
...
`Rule.options` is a shortcut to `Rule.use: [ { options } ]`. See [`Rule.use`](#ruleuse) and [`UseEntry.options`](#useentry) for details.

W> `Rule.query` was removed in webpack 5 and is rejected by the configuration validation. Use `Rule.options` or `UseEntry.options` instead.
...
`{ glob: string | [string] }`: <Badge text="5.110.0+" /> The input must match the glob pattern, or one of them. See [`Rule.glob`](#ruleglob) for the pattern syntax.

The input a condition is tested against depends on which field it is written in: [`test`](#ruletest), [`include`](#ruleinclude) and [`exclude`](#ruleexclude) all get the module's resource path, [`issuer`](#ruleissuer) the path of the module that requested it, [`resourceQuery`](#ruleresourcequery) the query string, and so on. The grammar is the same everywhere.

Conditions compose. A bare array is an `or`, so `test: [/\.ts$/, /\.tsx$/]` and `test: { or: [/\.ts$/, /\.tsx$/] }` match the same modules, while the properties of one object are combined with `and`. The following condition matches a TypeScript file under `src/` that is not a test file — the three properties must all hold, and `not` takes a condition of its own, so listing two regexps there excludes a file matching either of them:
...
T> `{ not: [A, B] }` means "neither `A` nor `B` matches", not "does not match both". To exclude only the inputs matching every one of several conditions, negate the `and` instead: `{ not: { and: [A, B] } }`.
...
These options live under [`module.parser.javascript`](#moduleparserjavascript). Here they are with their [defaults](https://github.com/webpack/webpack/blob/main/lib/config/defaults.js):
...
W> Setting any of these directly on `module` — `module.exprContextCritical` and the rest — is deprecated. Each one moved to `module.parser.javascript` under the same name, so `module.exprContextRegExp` becomes `module.parser.javascript.exprContextRegExp`. `module.strictExportPresence` moved the same way and is itself deprecated in favour of [`exportsPresence`](#moduleparserjavascriptexportspresence), and `module.strictThisContextOnImports` became [`module.parser.javascript.strictThisContextOnImports`](#moduleparserjavascriptstrictthiscontextonimports).
