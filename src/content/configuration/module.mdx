...
Cache the resolving of module requests. There are couple of defaults for `module.unsafeCache`:

- `false` if cache is disabled.
- `true` if cache is enabled and the module appears to come from node_modules otherwise `false`.
