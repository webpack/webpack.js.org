  ...
  module: {
    generator: {
      ...
      "css/auto": {
        ...
        // منذ webpack 5.108.0، يشير [hash] هنا إلى تجزئة المعرّف المحلي، كما في css-loader؛ استخدم [modulehash] لتجزئة الوحدة. ويمكن أيضًا تحديد ترميز التجزئة وطولها ضمن الصيغة نفسها، مثل [hash:base64:8].
      ...
      html: {
        // خيارات المولد لوحدات HTML. تتطلب `experiments.html`، ومتاحة منذ webpack 5.107.0.

        // إخراج HTML بعد تحليله وإعادة كتابة روابطه في ملف .html مستقل.
...
### أسماء مستعارة متعددة عبر `exportsConvention` {#multiple-aliases-via-exportsconvention}
...
export default {
  module: {
    parser: {
      ...
      html: {
        // خيارات محلل وحدات HTML. تتطلب `experiments.html`، ومتاحة منذ webpack 5.108.0.

        // تعطيل استخراج سمات URL أو تخصيصه.
...
- النوع: `boolean | 'preserve-unknown' | object`
- الافتراضي: `true`
- متاح: 5.68.0+
...
- `object` <Badge text="5.109.0+" /> - يتيح تفعيل تقييم حقول `import.meta` أو تعطيله لكل حقل على حدة. تبقى الحقول غير المذكورة مفعّلة، ويُحتفظ بالحقول غير المعروفة. المفاتيح المعروفة هي `dirname` و`env` و`filename` و`main` و`resolve` و`url` و`webpack` و`webpackContext` و`webpackHot`. ويمكن كذلك إضافة مفاتيح مخصصة للتحكم في تقييم حقولك الخاصة.
...
W> هذا الخيار مهمل منذ webpack 5.109.0. استخدم بدلًا منه خيار الكائن [`importMeta.webpackContext`](#moduleparserjavascriptimportmeta). عند ضبط الخيارين، تكون الأولوية لـ `importMeta.webpackContext`.
...
T> بدءًا من webpack 5.109.0، لم تعد المحللات المخصصة مطالبة بجمع `semicolons`، إذ تُستنتج مواضع الإدراج التلقائي للفواصل المنقوطة (ASI) من النص المصدري. كما أصبحت المحللات التي لا توفر واجهات لمعلومات الموقع مدعومة؛ حيث تُستنتج المواقع من إزاحات العُقد، فلا تحتاج عُقد AST إلى تضمين `loc`.
