...
The `relative` value for `module.parser.javascript.url` is available as of webpack <Badge text='5.23.0' />, it would generate relative URLs for `new URL()` syntax, i.e., there's no base URL included in the result:
...
1. This is useful for static site generators, mini-css-plugin and html-plugin, etc. where you would compile code first for server side rendering.
2. Also for SSR (Server side rendering) when base URL is not known by server (and it saves a few bytes). To be identical it must also be used for the client build.
