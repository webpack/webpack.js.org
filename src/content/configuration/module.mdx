...
When using `import`s in `.mjs` files or any `.js` files when their nearest parent `package.json` file contains a `"type"` field with a value of `"module"`, you're required to provide file extension, otherwise webpack would fail the compiling with `Module not found` error.
    ...
    rules: [
      {
        test: /\.m?js/,
        resolve: {
          fullySpecified: false // disable the enforcement
        }
      }
    ]
  }
