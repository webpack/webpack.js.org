...
When using `import`s in `.mjs` files or any other `.js` files when their nearest parent `package.json` file contains a `"type"` field with a value of `"module"`, webpack won't resolve against the [`resolve.mainFiles`](/configuration/resolve/#resolvemainfiles), and it's required to provide the file extension, otherwise webpack would fail the compiling with a `Module not found` error.
