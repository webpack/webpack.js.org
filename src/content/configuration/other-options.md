...
`string = 'node_modules/.cache/webpack'`
...
Algorithm used the hash generation. See [Node.js crypto](https://nodejs.org/api/crypto.html) for more details.
...
```string = `${config.name}-${config.mode}` ```

Name for the cache. Different names will lead to different coexisting caches. Using `cache.name` makes sense when you have multiple configurations which should have independent caches.
