...
Whether webpack validates the configuration, and the options passed to loaders and plugins, against their schemas. Defaults to `true`, except in a production build with [`experiments.futureDefaults`](/configuration/experiments/#experimentsfuturedefaults) enabled, where it defaults to `false`.
...
T> When validation is on, webpack calls the [`validate` hook](/api/compiler-hooks/#validate) so plugins can check their own options at the same point.
