...
Only appends lines to the output. Avoids updating existing output e. g. for status messages. This option is only used when no custom [`console`](#console) is provided.
...
Enable/Disable colorful output. This option is only used when no custom [`console`](#console) is provided.
...
Stream used for logging output. Defaults to `process.stderr`. This option is only used when no custom [`console`](#console) is provided.
