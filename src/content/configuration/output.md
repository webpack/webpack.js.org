...
W> an empty `output.library` with this libraryType will result in a broken output bundle.
...
`libraryTarget: "window"` - When your library is loaded, **the return value of your entry point** will be part `window` object under the property named by `output.library`.
...
`libraryTarget: "global"` - When your library is loaded, **the return value of your entry point** will be part `global` object under the property named by `output.library`.
...
`libraryTarget: "commonjs"` - When your library is loaded, **the return value of your entry point** will be part of the `exports` object under the property named by `output.library`. As the name implies, this is used in CommonJS environments:
...
### Exposing the library under formalized module definition systems

These options will result in a bundle that comes with a more complete header to ensures compatibility with various module systems.  The `output.library` option will take on a different meaning under the following `output.libraryTarget` options.
