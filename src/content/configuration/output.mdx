...
Note that we're passing a single entry file to `entry` in the above example. However webpack can accept [many kinds of entry point](/configuration/entry-context/#entry), e.g., an `array`, an `object`.
...
2. If an `object` is provided as the `entry` point, all can be exposed using the `array` syntax of `library`:
   ...
   Note that the above configuration won't work if you're going to configure library options per entry point. Here is how to do it [right under your entry point](/concepts/entry-points/#entrydescription-object):
...
Configure how the library will be exposed. Any one of the following options can be used. Please note that this option works in conjunction with the value assigned to [`output.library.name`](#outputlibraryname).
...
module.exports = {
  ...
  output: {
    library: {
      // note that you should not specify a name here
      type: 'module',
    },
  },
};
...
W> While the data this comments can provide is useful during development when reading the generated code, it **should not** be used in production.
