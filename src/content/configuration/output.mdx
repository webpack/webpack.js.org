...
Use a container(defined in global space) for call `define`/`require` functions in the AMD module.

W> Note that value of `amdContainer` **must be** set as global variable.
...
Which would result bundle like below:

```js
window['clientContainer'].define(...)... // or 'amd-require' window['clientContainer'].require(...)...
```
