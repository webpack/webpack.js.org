...
Set it to an object to configure how the injected `<script>` tags load:

| Option          | Description                                                           |
| --------------- | --------------------------------------------------------------------- |
| `scriptLoading` | `'auto'` (default), `'defer'`, or `'blocking'`. See the values below. |

- `'auto'`: emit a module script (`type="module"`) for ES module output and a deferred script otherwise.
