...
For assets built from data URI modules, `[file]`, `[query]`, `[fragment]` and `[path]` are set to an empty string, `[name]` and `[base]` resolve to the content hash, and `[ext]` is derived from the MIME type.
...
T> Since webpack 5.111.0, an [`output.module`](#outputmodule) build that loads chunks through a native `import()` names each one with a literal specifier held by the chunk loader, so a bundler or a preload scanner can follow it without running webpack's runtime. Those names live in the chunk carrying the runtime, so changing a lazily loaded chunk invalidates that one rather than every module importing it — webpack 5.110 wrote the specifier at each import site instead, which put the imported chunk's `[contenthash]` into the importing module.

By default the value is inferred from [`output.filename`](#outputfilename): it is used as-is when it contains `[name]`, `[id]`, `[chunkhash]` or `[contenthash]`; otherwise `[id].` is prepended to its basename (`bundle.js` becomes `[id].bundle.js`). When `output.filename` is a function, `[id].js` (or `[id].mjs` with [`output.module`](#outputmodule)) is used.
...
These capability flags are normally inferred from your [`target`](/configuration/target/); you only set them manually to override that detection. When a flag is enabled, webpack emits the corresponding modern syntax in its generated runtime code and otherwise falls back to the equivalent legacy output.
...
| [uniqueName] | The value of [`output.uniqueName`](#outputuniquename) (alias `[uniquename]`, 5.108.0+) |
...
With [`experiments.futureDefaults`](/configuration/experiments/#experimentsfuturedefaults) enabled, `[path]` and `[file]` already behave this way for asset and HTML modules, so the contained placeholders are what those templates resolve to anyway. This will become the default in webpack 6.
...
Customize the main hot update filename. `[fullhash]` and `[runtime]` are available as placeholder. When [`output.chunkLoading`](#outputchunkloading) is `'import'`, the default is `'[runtime].[fullhash].hot-update.json.mjs'`, because the manifest is loaded via `import()`.
...
Generate an HTML file for each non-HTML [entrypoint](/concepts/entry-points/), injecting that entrypoint's initial JS and CSS output chunks (including chunks shared through [`dependOn`](/configuration/entry-context/#dependencies)). This is the document scaffolding that used to need a separate plugin, built into webpack core. It relies on the experimental HTML support, so enable [`experiments.html`](/configuration/experiments/#experimentshtml).
...
Stylesheet `<link>` tags for CSS chunks are not affected: they always land in `<head>` when the page has one, ahead of the first blocking script, or after `defer` / module script tags when there is none.
...
W> This is not [`output.library.amdContainer`](#outputlibraryamdcontainer), which applies to the `amd` and `amd-require` library types and calls `define`/`require` through the container. `umdAmdContainer` only adds a branch to the `umd` wrapper's detection chain.
...
If `output.library` is set, the return value of your entry point is assigned to `module.exports.<name>` instead, see [`type: 'commonjs2'`](#type-commonjs2).
...
With this target, the library name must be unset; webpack throws an error otherwise.
...
Output JavaScript files as module type. It defaults to `true` for the [`universal`](/configuration/target/#universal), [`deno`](/configuration/target/#deno) and [`bun`](/configuration/target/#bun) targets, which only run ECMAScript modules, and for a [`module`](#type-module) or [`modern-module`](#type-modern-module) library type, which exists only as an ECMAScript module. It defaults to `false` everywhere else.

Under [`experiments.futureDefaults`](/configuration/experiments/#experimentsfuturedefaults) <Badge text="5.111.0+" /> it defaults to `true` wherever the target can read an ECMAScript module, which is the webpack 6 default. Three things keep script output there:

- a [`target`](/configuration/target/) stating it cannot read one — a `browserslist` query resolving to engines without module support (`ie 11`), or a version below it (`node10`, `['web', 'es5']`). A target that says nothing about modules, such as `web`, `webworker` or a plain `node`, counts as able to read one;
- a library type that is read out of a script — `var`, `umd`, `commonjs2`, `assign`, `window`, `global`, … — wherever it is declared: [`output.library`](#outputlibrary), an entry's own `library`, or `output.enabledLibraryTypes` as a plugin such as `ModuleFederationPlugin` leaves it;
- setting `output.module` yourself, which wins over the target and the library type in both directions.
...
T> [`performance.conflictingResourceHints`](/configuration/performance/#performanceconflictingresourcehints) reports the other half of the problem: a chunk asked for as both prefetch and preload from the same place, where the two directives contradict each other.
...
Option to set the method of loading WebAssembly Modules. Methods included by default are `'fetch'` (web/WebWorker), `'async-node'` (Node.js), `'universal'` (both), but others might be added by plugins.
