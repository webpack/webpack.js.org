...
Configure how the library will be exposed. Any one of the following options can be used. Please note that this option works in conjunction with the value assigned to [`output.library.name`](#outputlibraryname). For the following examples, it is assumed that the value of [`output.library.name`](#outputlibraryname) is configured as `MyLibrary`.
...
W> Please use [`output.library.type`](#outputlibrarytype) instead as we might drop for `output.libraryTarget` in the future.
