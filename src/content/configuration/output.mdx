...
T> Since webpack 5.111.0, an [`output.module`](#outputmodule) build that loads its chunks through a native `import()` names each one with a literal specifier held by the chunk loader, so a bundler or a preload scanner can follow it without running webpack's runtime, and no chunk id is turned into a filename. The names live in the chunk carrying the runtime — with [`optimization.runtimeChunk`](/configuration/optimization/#optimizationruntimechunk) that is the runtime chunk alone — so changing a lazily loaded chunk invalidates that one rather than every module importing it. Webpack 5.110 wrote the specifier at each import site instead, which put the imported chunk's `[contenthash]` into the importing module.
