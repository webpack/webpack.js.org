...
When enabled, webpack will set [`output.iife`](#outputiife) to `false`, [`output.scriptType`](#outputscripttype) to `'module'` and `terserOptions.module` to `true` internally. In addition, if you have [`output.library`](#outputlibrary) set, i.e., you're using webpack to compile a library to be consumed by others, webpack will also set [`output.libraryTarget`](#outputlibrarytarget) to `'module'`.
