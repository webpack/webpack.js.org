...
T> To ignore modules based on a regex, you can use [IgnorePlugin](https://webpack.js.org/plugins/ignore-plugin/)
...
If unsafe cache is enabled, includes `request.context` in the cache key. This option is taken into account by the [`enhanced-resolve`](https://github.com/webpack/enhanced-resolve/) module. Set it to `false` to exclude the context from the cache key.
...
Enable aggressive, but **unsafe**, caching of modules. Disabled unless set; passing `true` will cache everything.
...
T> This option is also available in `resolveLoader` with the same configuration options.
...
T> Note that you can use alias here and other features familiar from resolve. For example `{ txt: 'my-text-loader' }` would shim `txt!templates/demo.txt` to use `my-text-loader`.
