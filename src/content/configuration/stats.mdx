...
| `'detailed'`        | _none_      | Like `'verbose'` but without `chunkModules`, `chunkGroupResourceHints`, `dependentModules`, `reasons`, `orphanModules`, `errorStack`, `env` and `children`, and with `errorsSpace`, `warningsSpace`, `modulesSpace`, `assetsSpace` and `reasonsSpace` limited to 1000 |
...
T> The defaults below describe the string output (`stats.toString()`, used by the CLI). These options are `false` there but `true` in `stats.toJson()`: `builtAt`, `cachedAssets`, `cachedModules`, `children`, `chunkGroupAuxiliary`, `chunkGroupChildren`, `chunkGroupResourceHints`, `chunkGroups`, `chunkOrigins`, `chunkRelations`, `chunks`, `dependentModules`, `depth`, `errorStack`, `hash`, `ids`, `loggingTrace`, `moduleAssets`, `nestedModules`, `optimizationBailout`, `orphanModules`, `outputPath`, `providedExports`, `publicPath`, `reasons`, `relatedAssets`, `runtimeModules` and `usedExports`.
...
Tells `stats` to sort the assets by a given field. All of the [sorting fields](#sorting-fields) are allowed to be used as values for `stats.assetsSort`. Use `!` prefix in the value to reverse the sort order by a given field. Set it to `false` to disable sorting.
...
Tells `stats` to sort the chunks by a given field. All of the [sorting fields](#sorting-fields) are allowed to be used as values for `stats.chunksSort`. Use `!` prefix in the value to reverse the sort order by a given field. Sorting is disabled by default (`false`).
...
Tells `stats` whether to display the entry points with the corresponding bundles. Defaults to `'auto'` in the string output and to `true` in `stats.toJson()`.
...
W> Deprecated and ineffective: `stats.exclude` is still accepted by the configuration schema but never read. Use [`stats.excludeModules`](#statsexcludemodules) instead.
...
Add the performance hints reported with [`performance.hints: "stats"`](/configuration/performance/#performancehints). With that value the hints are not emitted as warnings or errors, so the stats are where they are read.
...
Tells `stats` to sort the modules by a given field. All of the [sorting fields](#sorting-fields) are allowed to be used as values for `stats.modulesSort`. Use `!` prefix in the value to reverse the sort order by a given field. Set it to `false` to disable sorting.
...
Tells `stats` whether to show `orphan` modules. A module is an `orphan` if it is not included in any chunk. Orphan modules are hidden by default in `stats`.
...
Setting the top-level `stats` option to `false` tells webpack to use the `'none'` [stats preset](/configuration/stats/#stats-presets); inside the `stats` object, use `preset: 'none'`.
