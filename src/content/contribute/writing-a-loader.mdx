...
Webpack takes it from there: the HTML that falls out of the chain is handled by [built-in HTML support](/guides/native-html/), so nothing needs to turn it into a module.
...
A CSS loader is a good example of the first approach: it transforms dependencies to `require`s, replacing `@import` statements with a `require` to the other stylesheet and `url(...)` with a `require` to the referenced file. Webpack now does this itself — see [Native CSS](/guides/native-css/).
