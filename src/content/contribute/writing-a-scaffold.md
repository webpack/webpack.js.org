...
Sweet! We've now created a scaffold with `entry`, `output`, `context` and a `plugin`. If you're curious on the API, check the [API](/guides/scaffoldfing.md) for more info on how to scaffold with `regexps`, `module`, or other!
