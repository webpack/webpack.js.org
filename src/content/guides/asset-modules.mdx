...
When [`experiments.futureDefaults`](/configuration/experiments/#experimentsfuturedefaults) is enabled, webpack registers the rules above for you: the query suffixes `?raw`, `?url`, `?inline` and `?no-inline` work out of the box for any import, similar to Vite's asset queries.
...
The suffixes are matched anywhere in the query string (`?foo&raw` works too) as a `oneOf` list, so only the first matching suffix wins. Since they are plain default rules, your own `module.rules` take precedence and can override them.
