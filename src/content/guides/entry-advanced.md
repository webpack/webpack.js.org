...
It is possible to provide different types of files when using arrays for [entry](/configuration/entry-context/#entry) value(s) to achieve separate bundles for CSS and JavaScript (and other) files in applicataions that are not using `import` for styles in JavaScript (pre Single Page Applications or different reasons).
