...
webpack 5.x.x compiled successfully in 3064 ms
```
...
T> The page itself emits no JavaScript, so the scripts it pulls in take the entry's filename: `main.bundle.js` for the first, `main1.bundle.js` for the second. If you would rather keep named JavaScript entries and have webpack scaffold a document around each one, set [`output.html`](/configuration/output/#outputhtml) instead; the [Native HTML](/guides/native-html/) guide walks through both models, along with titles, meta tags, favicons and templating.
