...
W> `imports-loader` is deprecated and only receives bug fixes. It still works, and the example below is unchanged, but webpack can do the same from the configuration itself — see webpack's [add-imports example](https://github.com/webpack/webpack/tree/main/examples/add-imports), which is the same behaviour as a short plugin.
...
This becomes a problem when the module is executed in a CommonJS context where `this` is equal to `module.exports`. In this case you can override `this` using the [`imports-loader`](https://github.com/webpack/imports-loader):
...
W> `exports-loader` is deprecated and only receives bug fixes. It still works, and the example below is unchanged, but appending exports to a file that has none is a few lines over webpack's public API — see webpack's [add-exports example](https://github.com/webpack/webpack/tree/main/examples/add-exports).
...
Now, while you'd likely never do this in your own source code, you may encounter a dated library you'd like to use that contains similar code to what's shown above. In this case, we can use [`exports-loader`](https://github.com/webpack/exports-loader), to export that global variable as a normal module export. For instance, in order to export `file` as `file` and `helpers.parse` as `parse`:
