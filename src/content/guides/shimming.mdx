...
This becomes a problem when the module is executed in a CommonJS context, where the module's own `this` is `module.exports` rather than the global object. Wrapping the module in a function called with the global object fixes that, and it needs no loader: `NormalModule`'s `processResult` hook hands a plugin what the loaders produced and takes back a replacement, so putting code around a file before webpack parses it is a short plugin you can keep in the configuration.
...
+const PLUGIN_NAME = 'SurroundModulePlugin';
+
+// A prepend before a directive would demote it to an expression, silently
+// making the module sloppy, so the code goes after one.
+const DIRECTIVE = /^\s*(["'])use strict\1;?/;
+
+class SurroundModulePlugin {
+  constructor(rules) {
+    this.rules = rules;
+  }
+
...
+
+          for (const [resource, { before = '', after = '' }] of this.rules) {
+            if (module.resource !== resource) continue;
+
...
+
+            // no newline after `before`: one would shift every line below it
+            // out of the source map
+            return [
...
+
 export default {
   ...
   plugins: [
+    new SurroundModulePlugin([
+      [
+        fileURLToPath(import.meta.resolve('./src/index.js')),
+        { before: '(function () {', after: '}).call(globalThis);' },
+      ],
+    ]),
     new webpack.ProvidePlugin({
       join: ['lodash', 'join'],
...
W> The wrapper is for scripts only. `import` and `export` may appear only at the top level, so wrapping an ES module in a function is a syntax error — and an ES module's own `this` is `undefined` anyway, never `module.exports`.

T> A name the module only _reads_ is [`ProvidePlugin`](/plugins/provide-plugin/)'s job, since it binds the name where the module reads it without touching the source. What is left for this plugin is the wrapper above, a side-effect import the file never names, and a real binding. webpack's [add-imports example](https://github.com/webpack/webpack/tree/main/examples/add-imports) is the same plugin with a `RegExp` condition, so one entry can match many files.
...
Now, while you'd likely never do this in your own source code, you may encounter a dated library you'd like to use that contains similar code to what's shown above. A file that exports nothing needs the exports appending to it, which is the same plugin from the other end of the file — only `after` this time. For instance, in order to export `file` as `file` and `helpers.parse` as `parse`:
...
+      [
+        fileURLToPath(import.meta.resolve('./src/globals.js')),
+        { after: 'module.exports = { file, parse: helpers.parse };' },
+      ],
     ]),
     new webpack.ProvidePlugin({
       join: ['lodash', 'join'],
...
T> Which format to append is the file's own, and the two are not equivalent. `module.exports = …` keeps a script the script it is, which is what `globals.js` above wants. `export { … }` is what makes a file an ES module, exactly as it would be in the source, so those exports can be analyzed and tree-shaken — prefer it wherever the file tolerates it. webpack's [add-exports example](https://github.com/webpack/webpack/tree/main/examples/add-exports) shows both side by side.
...
Lastly, there are some modules that support multiple [module styles](/concepts/modules); e.g. a combination of AMD, CommonJS, and legacy. In most of these cases, they first check for `define` and then use some quirky code to export properties. In these cases, it could help to force the CommonJS path by prepending `var define = false;` with the [plugin above](#granular-shimming) — `{ before: 'var define = false;' }` for that file.
