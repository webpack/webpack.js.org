...
This becomes a problem when the module is executed in a CommonJS context, where the module's own `this` is `module.exports` rather than the global object. Wrapping the module in a function called with the global object fixes that, and it needs no loader: `NormalModule`'s `processResult` hook hands a plugin what the loaders produced and takes back a replacement, so putting code around a file before webpack parses it is a plugin short enough to keep in the configuration.

webpack's [add-imports example](https://github.com/webpack/webpack/tree/main/examples/add-imports) is that plugin. It covers the wrapper above, a side-effect import the file never names, and a real binding.
...
T> A name the module only _reads_ is [`ProvidePlugin`](/plugins/provide-plugin/)'s job, since it binds the name where the module reads it without touching the source.
...
Now, while you'd likely never do this in your own source code, you may encounter a dated library you'd like to use that contains similar code to what's shown above. A file that exports nothing needs the exports appending to it, which is the same hook from the other end of the file: webpack's [add-exports example](https://github.com/webpack/webpack/tree/main/examples/add-exports) is the plugin for it.

For `globals.js` above, appending `module.exports = { file, parse: helpers.parse };` exports `file` as `file` and `helpers.parse` as `parse`. From within our entry script (i.e. `src/index.js`), we could then use `const { file, parse } = require('./globals.js');` and all should work smoothly.

T> Which format to append is the file's own, and the two are not equivalent. `module.exports = …` keeps a script the script it is, which is what `globals.js` above wants. `export { … }` is what makes a file an ES module, exactly as it would be in the source, so those exports can be analyzed and tree-shaken — prefer it wherever the file tolerates it. The example shows both side by side.
...
Lastly, there are some modules that support multiple [module styles](/concepts/modules); e.g. a combination of AMD, CommonJS, and legacy. In most of these cases, they first check for `define` and then use some quirky code to export properties. In these cases, it could help to force the CommonJS path by prepending `var define = false;` to that file, with the same [add-imports example](https://github.com/webpack/webpack/tree/main/examples/add-imports) plugin used under [Granular Shimming](#granular-shimming).
