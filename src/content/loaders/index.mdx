...
T> Webpack has experimental built-in HTML support ([`experiments.html`](/configuration/experiments/#experimentshtml)): it parses `.html`, extracts the URLs a page references and bundles inline `<script>` / `<style>`, so importing a plain HTML partial needs no loader. Template engines still need theirs — [What's built-in](/guides/native-html/#whats-built-in) states the exact scope.

- [`html-loader`](/loaders/html-loader) Exports HTML as string, require references to static resources. Webpack can [do this itself](/guides/native-html/#3-html-imported-from-javascript); still needed for `sources.scriptingEnabled` and `postprocessor`
...
T> Webpack has experimental built-in CSS support ([`experiments.css`](/configuration/experiments/#experimentscss)): it parses `.css`, resolves `@import` and `url()`, handles CSS Modules, extracts or injects stylesheets and minifies them, so a setup using the CSS loaders with default options needs neither. It does **not** replace preprocessors, and a handful of loader options have no equivalent — [What's built-in](/guides/native-css/#whats-built-in) states the exact scope.

- [`style-loader`](/loaders/style-loader) Add exports of a module as style to DOM. Webpack can [inject styles itself](/guides/native-css/#output-modes-exporttype); still needed for `insert`, `attributes`, `styleTagTransform` and lazy / singleton injection
- [`css-loader`](/loaders/css-loader) Loads CSS file with resolved imports and returns CSS code. Webpack can [parse CSS itself](/guides/native-css/); still needed for `importLoaders`, `localIdentRegExp`, `getJSON` and the `url` / `import` filter callbacks
