...
- Without a browserslists it would emit ES6 style. You can use `target: ["web", "es5"]` to change it to ES5.
- For Node.js builds include the supported Node.js version in the `target` option and webpack automatically figure out which syntax is supported, e. g. `target: "node8.6"`.
...
- When using `const compiler = webpack(...);`, make sure to close the compiler after using it: `compiler.close(callback);`.
  - This doesn't apply to the `webpack(..., callback)` form. It will automatically call close.
