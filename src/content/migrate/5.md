...
你可以像如下的办法调用你的 webpack 以获取过期告警的栈堆叠追踪并推断出是由哪个插件或加载器引起的。

```bash
node --trace-deprecation node_modules/webpack/bin/webpack.js
```

W> webpack 5 会清除所有过期的特性。为了继续推进，应该在构建中再无任何的过期警告。
...
- 通过设置 `experiments.syncWebAssembly: true` 启动过期细则，获得在 webpack 4 中的相同行为。
...
过期警告。你可能会得到许多过期的警告。目前这不需要是一个问题。插件需要时间跟上webpack内核的
改变。请忽略这些警告直到发布候选版本推出而不再是Beta版本。

- 你使用 `--no-deprecation` 标志运行 node 进行，可以隐藏过期告警，例如：`node --no-deprecation node_modules/webpack/bin/webpack.js`。
这只能作为一个临时的变通方案。
- 插件和加载器贡献者，应该跟随过期建议信息以改进代码。
...
使用 JSON5 应该被认为且被写在文档中是已经过期的并推荐使用 JSON。
