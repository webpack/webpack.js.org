...
- The first argument of the `webpack.DefinePlugin.runtimeValue` is a `function(module, key, version)` that should return the value to be assigned to the definition.
...
- <Badge text="5.26.0+" /> The second argument can also take an options object argument
  with the following properties - `fileDependencies?: string[]` A list of files the
  function depends on. - `contextDependencies?: string[]` A list of directories the
  function depends on. - `missingDependencies?: string[]` A list of not existing
  files the function depends on. - `buildDependencies?: string[]` A list of build
  dependencies the function depends on. - `version?: string | () => string` A version
  of the function.
