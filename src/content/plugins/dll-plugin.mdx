...
Because this happens after resolving every file in the dll bundle, the same paths must be available for the consumer of the dll bundle. i.e. if the dll contains `lodash` and the file `abc`, `import _ from "lodash"`/`require('lodash')` and `import abs from "./abc"`/`require('./abc')` will be used from the dll, rather than building them into the main bundle.
