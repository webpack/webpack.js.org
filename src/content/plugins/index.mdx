...
T> CSS and HTML are built in (experimental). [`MiniCssExtractPlugin`](/plugins/mini-css-extract-plugin) is not listed here but stays documented for what the built-in CSS support does not cover — see [CSS](/guides/native-css/#whats-built-in). For HTML, the [Native HTML](/guides/native-html/) guide covers [what is built in](/guides/native-html/#whats-built-in) and how to move an existing page-generating setup over.
