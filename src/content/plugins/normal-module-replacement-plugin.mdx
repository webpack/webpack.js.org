...
Note that the `resourceRegExp` is tested twice: against the request as written in your code before resolving (e.g. `'./sum'` for `import sum from './sum'`) and against the resolved absolute resource path after resolving (e.g. `'/project/src/sum.js'`).
