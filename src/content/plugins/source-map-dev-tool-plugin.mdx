...
W> If the default webpack `minimizer` has been overridden (such as to customise the `MinimizerPlugin` options), make sure to configure its replacement with `sourceMap: true` to enable SourceMap support.
