...
Minimum size reduction to the whole compilation, in bytes, needed for a chunk to be generated.

T> Both `splitChunks.minSizeReduction` and `splitChunks.minSize` need to be fulfilled for a chunk to be generated. You may want to reduce `minSize` when using `minSizeReduction`.
