...
- New chunk would be bigger than 20kb (before min+gz; 10kb outside `production` mode)
- Maximum number of parallel requests when loading chunks on demand would be lower or equal to 30 (unlimited outside `production` mode)
- Maximum number of parallel requests at initial page load would be lower or equal to 30 (unlimited outside `production` mode)
...
This configuration object represents the default behavior of the `SplitChunksPlugin` in `production` [`mode`](/configuration/mode/); the comments give the values used in the other modes.
...
The default value of `splitChunks.maxAsyncRequests` depends on the [`mode`](/configuration/mode/):
...
The default value of `splitChunks.maxInitialRequests` depends on the [`mode`](/configuration/mode/):
...
The default value of `splitChunks.minSize` depends on the [`mode`](/configuration/mode/):
...
The default value of `splitChunks.enforceSizeThreshold` depends on the [`mode`](/configuration/mode/):
...
If `splitChunks.name` matches the name of an existing chunk (e.g. an [entry point](/configuration/entry-context/#entry)), the selected modules are moved into that chunk only when it is a parent of all selected chunks (e.g. via [`dependOn`](/configuration/entry-context/#dependencies)); otherwise webpack emits an error.
...
If the current chunk contains modules already split out from the main bundle, it will be reused instead of a new one being generated. This can affect the resulting file name of the chunk. The built-in `default` and `defaultVendors` cache groups set it to `true`.
...
- Condition 2: `react` is bigger than 20kb
...
- Condition 2: `helpers` is bigger than 20kb
- Condition 3: Number of parallel requests at the import calls is 2
- Condition 4: Doesn't affect request at initial page load

Putting the content of `helpers` into each chunk will result into its code being downloaded twice. By using a separate chunk this will only happen once. We pay the cost of an additional request, which could be considered a tradeoff. That's why there is a minimum size of 20kb.
