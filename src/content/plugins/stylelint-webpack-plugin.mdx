...
> stylelint-webpack-plugin 3.0 仅支持 webpack 5。对于 webpack 4，请查看 [2.x 分支](https://github.com/webpack-contrib/stylelint-webpack-plugin/tree/2.x)。
...
有关可用选项的完整列表请参阅 [styleint 的配置项](https://stylelint.io/user-guide/usage/node-api#options)，这些选项直接传递给 `stylelint`。
...
**注意：** 默认情况下由 [`stylelint` 处理](https://stylelint.io/user-guide/configure)。
...
如果值为 `true`，`stylelint` 将修复尽可能多的 error。修复真实的源文件。报告所有未修复的 errors。请参阅 [自动修复错误](https://stylelint.io/user-guide/usage/options#fix) 文档。
...
指定要用于格式化结果的 formatter。请参阅 [formatter 选项](https://stylelint.io/user-guide/usage/options#formatter)。
