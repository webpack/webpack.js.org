...
function fetchPackageNames(options, cb) {
  if (typeof options.organization !== 'string') {
    return console.error('utilities/fetch-package-names: missing organization');
  }

  if (typeof options.suffix !== 'string') {
    return console.error('utilities/fetch-package-names: missing suffix');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    github.repos.getForOrg({
      ...
      const filteredNames = response.data.filter((organization) => {
